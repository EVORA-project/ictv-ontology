...
def main():
    ...
    releases.loc[:, 'msl_release_num'] = releases['msl_release_num'].astype(int)
    releases = releases.sort_values(by='msl_release_num', ascending=False)

    last_release = releases.iloc[0]

    id = 'ictv_' + str(last_release.msl_release_num)
    ontology_iri = f'{prefix}{id}'
    g.add((URIRef(ontology_iri), RDF.type, OWL.Ontology))
    g.add((URIRef(ontology_iri), RDFS.label, Literal("ICTV Taxonomy (" + str(last_release.name) + ")")))
    ...
    g.add((URIRef(ontology_iri), OWL.versionInfo, Literal(str(last_release.msl_release_num))))
    ...
    for node in nodes.itertuples():
        ...
        if n % 10000 == 0:
            pc_complete = n / len(nodes) * 100
            timestamp  = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            print(f'{timestamp} Processing node {n} of {len(nodes)} ({pc_complete:.2f}%)')

        if node.level_id == '100':
            continue

        class_iri = URIRef(prefix + node.taxnode_id)
        ...
        g.add((class_iri, OWL.versionInfo, Literal(str(node.msl_release_num))))

        other_versions = nodes[(nodes['ictv_id'] == node.ictv_id) & (nodes['taxnode_id'] != node.taxnode_id)]
        older_versions = other_versions[other_versions['msl_release_num'] < node.msl_release_num]
        for older_version in older_versions.itertuples():
            if older_version.name != node.name:
                g.add((class_iri, URIRef(SYNONYM), Literal(older_version.name)))
                stmt = BNode()
                g.add((stmt, RDF.type, OWL.Axiom))
                g.add((stmt, OWL.annotatedSource, class_iri))
                g.add((stmt, OWL.annotatedProperty, URIRef(SYNONYM)))
                g.add((stmt, OWL.annotatedTarget, Literal(older_version.name)))
                g.add((stmt, URIRef(SYNONYM_TYPE), URIRef(PREVIOUS_NAME)))

        if node.parent_id != node.tree_id:
            g.add((class_iri, RDFS.subClassOf, URIRef(f'{prefix}{node.parent_id}')))
    ...
    owl_filename = 'out/ictv_all_versions.owl.ttl'
